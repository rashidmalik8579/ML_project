...
var riotRequest= new RiotRequest("RGAPI-b3eff757-40f8-4519-9574-376d3d9aecf8");
var p="";
...
///////////////////////
const summonerdata1=  mongoose.model("summonerdata1", summoner_schema);
const matchdata1 = mongoose.model("matchdata1", summoner_schema); 
...
async function savesummonerdatafrompuuid(puuid,first_summoner_puuid_list_doc,){
        ...
        let temp_doc=await summonerdata1.findOne({"summoner_by_name.puuid":first_summoner_puuid_list_doc.match_timeline.metadata.participants[i]});
...
async function savesummonerdata(temp=[]){
                    ...
                    const firstsummoner = new summonerdata1({
                        name:temp.name,
...
app.post("/", function (req, res) {                
    ...
    async function order(){         
        ...
        const matchdata_doc= await matchdata1.find({},"match_data.metadata.participants").exec();
            //const summonerdoc=await summonerdata.find();
        let checklist=[];
        const summonerdoc=await summonerdata1.find({},"summoner_by_name.puuid").exec();
        ...
        for(u=0;u<matchdata_doc.length;u++){
            ...
            //////////////////////////////////////////////  
            for(x=0;x<10;x++){
                    var flag=  await (summonerdata1.findOne({"summoner_by_name.puuid":puuidlist[x]}))
                    if(Boolean(flag)){
                    ...
                    //await savesummonerdata(c);
                    //console.log("summoner data saved successfullyyyy bsdk");           
                            
                    
           // checklist=puuidlist; //checking with recent 10 participants for duplication
            //console.log("checklist updated!! length: ",checklist.length)
